...
SRTSDKScrollBox::FSlot::FSlotArguments SRTSDKScrollBox::Slot()
{
	return FSlot::FSlotArguments(MakeUnique<FSlot>());
}
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void SRTSDKScrollPanel::Construct(const FArguments& InArgs, const TArray<SRTSDKScrollBox::FSlot*>& InSlots)
{
	PhysicalOffset = 0;
...
void SRTSDKScrollPanel::Construct(const FArguments& InArgs, TArray<SRTSDKScrollBox::FSlot::FSlotArguments> InSlots)
{
	PhysicalOffset = 0;
...
void SRTSDKScrollPanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
	...
	{
		const SRTSDKScrollBox::FSlot& ThisSlot = Children[SlotIndex];
...
FVector2D SRTSDKScrollPanel::ComputeDesiredSize(float) const
{
...
float SRTSDKScrollPanel::ArrangeChildVerticalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SRTSDKScrollBox::FSlot& ThisSlot, float CurChildOffset) const
{
...
float SRTSDKScrollPanel::ArrangeChildHorizontalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SRTSDKScrollBox::FSlot& ThisSlot, float CurChildOffset) const
{
	const FMargin& ThisPadding = ThisSlot.GetPadding();
	const FVector2D& WidgetDesiredSize = ThisSlot.GetWidget()->GetDesiredSize();
...
SRTSDKScrollBox::SRTSDKScrollBox()
{
...
void SRTSDKScrollBox::Construct(const FArguments& InArgs)
{
	check(InArgs._Style);
	...
	NotchDistance = InArgs._NotchDistance;

	if (InArgs._ExternalScrollbar.IsValid())
	{
		// An external scroll bar was specified by the user
		ScrollBar = InArgs._ExternalScrollbar;
		ScrollBar->SetOnUserScrolled(FRTSDKOnUserScrolled::CreateSP(this, &SRTSDKScrollBox::ScrollBar_OnUserScrolled));
	...
	else
	{
		...
		ScrollBar->SetMovesNotched(MovesNotched);
		ScrollBar->SetNotchOffset(GetNotchOffsetFraction());

		bScrollBarIsExternal = false;
	}

	SAssignNew(ScrollPanel, SRTSDKScrollPanel, MoveTemp(const_cast<TArray<FSlot::FSlotArguments>&>(InArgs._Slots)))
...
void SRTSDKScrollBox::OnClippingChanged()
{
	ScrollPanel->SetClipping(Clipping);
}

TSharedPtr<SRTSDKScrollBar> SRTSDKScrollBox::ConstructScrollBar()
{
	return TSharedPtr<SRTSDKScrollBar>(SNew(SRTSDKScrollBar)
		.Style(ScrollBarStyle)
		.Orientation(Orientation)
		.Padding(0.0f)
		.OnUserScrolled(this, &SRTSDKScrollBox::ScrollBar_OnUserScrolled));
}

void SRTSDKScrollBox::ConstructVerticalLayout()
{
	...
	[
		...
		[
			...
			[
				...
				.ColorAndOpacity(this, &SRTSDKScrollBox::GetStartShadowOpacity)
			...
			[
				...
				SNew(SImage)
				.Visibility(EVisibility::HitTestInvisible)
				.ColorAndOpacity(this, &SRTSDKScrollBox::GetEndShadowOpacity)
...
void SRTSDKScrollBox::ConstructHorizontalLayout()
{
...
/** Adds a slot to SRTSDKScrollBox */
SRTSDKScrollBox::FScopedWidgetSlotArguments SRTSDKScrollBox::AddSlot()
{
...
void SRTSDKScrollBox::RemoveSlot(const TSharedRef<SWidget>& WidgetToRemove)
{
	ScrollPanel->Children.Remove(WidgetToRemove);
}

void SRTSDKScrollBox::ClearChildren()
{
	ScrollPanel->Children.Empty();
}

bool SRTSDKScrollBox::IsRightClickScrolling() const
{
...
float SRTSDKScrollBox::GetScrollOffset() const
{
	return DesiredScrollOffset;
}

float SRTSDKScrollBox::GetScrollOffsetOfEnd() const
{
...
bool SRTSDKScrollBox::GetMovesNotched() const
{
	return MovesNotched;
}

float SRTSDKScrollBox::GetNotchDistance() const
{
	return NotchDistance;
}

float SRTSDKScrollBox::GetViewFraction() const
{
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
...
float SRTSDKScrollBox::GetViewOffsetFraction() const
{
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
...
float SRTSDKScrollBox::GetNotchOffsetFraction() const
{
	if (ScrollPanel.IsValid())
	{
		...
		const float ViewFraction = GetViewFraction();
		return FMath::Clamp<float>(NotchDistance / ContentSize, 0.0, 1.0 - ViewFraction);
	}
	return 0;
}

void SRTSDKScrollBox::SetScrollOffset(float NewScrollOffset)
{
...
void SRTSDKScrollBox::ScrollToStart()
{
	SetScrollOffset(0);
}

void SRTSDKScrollBox::ScrollToEnd()
{
	bScrollToEnd = true;
...
void SRTSDKScrollBox::ScrollDescendantIntoView(const TSharedPtr<SWidget>& WidgetToScrollIntoView, bool InAnimateScroll, ERTSDKDescendantScrollDestination InDestination, float InScrollPadding)
{
...
bool SRTSDKScrollBox::InternalScrollDescendantIntoView(const FGeometry& MyGeometry, const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll, ERTSDKDescendantScrollDestination InDestination, float InScrollPadding)
{
	...
	if (WidgetToFind.IsValid())
	{
		...
		else
		{
			...
			if (InDestination == ERTSDKDescendantScrollDestination::TopOrLeft)
			{
			...
			else if (InDestination == ERTSDKDescendantScrollDestination::BottomOrRight)
			{
			...
			else if (InDestination == ERTSDKDescendantScrollDestination::Center)
			{
				// Calculate how much we would need to scroll to bring this to the top/left of the scroll box
...
EOrientation SRTSDKScrollBox::GetOrientation()
{
	return Orientation;
}

void SRTSDKScrollBox::SetNavigationDestination(const ERTSDKDescendantScrollDestination NewNavigationDestination)
{
	NavigationDestination = NewNavigationDestination;
}

void SRTSDKScrollBox::SetConsumeMouseWheel(EConsumeMouseWheel NewConsumeMouseWheel)
{
	ConsumeMouseWheel = NewConsumeMouseWheel;
}

void SRTSDKScrollBox::SetOrientation(EOrientation InOrientation)
{
...
void SRTSDKScrollBox::SetScrollBarVisibility(EVisibility InVisibility)
{
	ScrollBar->SetUserVisibility(InVisibility);
}

void SRTSDKScrollBox::SetScrollBarAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarAlwaysVisible(InAlwaysVisible);
}

void SRTSDKScrollBox::SetScrollBarTrackAlwaysVisible(bool InAlwaysVisible)
{
	ScrollBar->SetScrollBarTrackAlwaysVisible(InAlwaysVisible);
}

void SRTSDKScrollBox::SetScrollBarThickness(FVector2D InThickness)
{
	ScrollBar->SetThickness(InThickness);
}

void SRTSDKScrollBox::SetScrollBarPadding(const FMargin& InPadding)
{
...
void SRTSDKScrollBox::SetScrollBarRightClickDragAllowed(bool bIsAllowed)
{
	bAllowsRightClickDragScrolling = bIsAllowed;
}

void SRTSDKScrollBox::SetMovesNotched(bool InMovesNotched)
{
	MovesNotched = InMovesNotched;
	if (ScrollBar.IsValid())
	{
		ScrollBar->SetMovesNotched(InMovesNotched);
	}
}

void SRTSDKScrollBox::SetNotchDistance(float InNotchDistance)
{
...
EActiveTimerReturnType SRTSDKScrollBox::UpdateInertialScroll(double InCurrentTime, float InDeltaTime)
{
...
void SRTSDKScrollBox::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	const float ViewFraction = GetViewFraction();
	if (LastViewFraction != ViewFraction)
	{
		LastViewFraction = ViewFraction;
		if (ScrollBar.IsValid())
		{
			ScrollBar->SetNotchOffset(GetNotchOffsetFraction());
		}
...
bool SRTSDKScrollBox::ComputeVolatility() const
{
	return bIsScrolling || IsRightClickScrolling();
}

FReply SRTSDKScrollBox::OnPreviewMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SRTSDKScrollBox::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SRTSDKScrollBox::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SRTSDKScrollBox::OnMouseMove(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SRTSDKScrollBox::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if (MouseEvent.IsTouchEvent())
	{
		if (!bFingerOwningTouchInteraction.IsSet())
...
void SRTSDKScrollBox::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	if (HasMouseCapture() == false)
	{
...
FReply SRTSDKScrollBox::OnMouseWheel(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	if ((ScrollBar->IsNeeded() && ConsumeMouseWheel != EConsumeMouseWheel::Never) || ConsumeMouseWheel == EConsumeMouseWheel::Always)
	{
		...
		{
			const bool bScrollWasHandled = ScrollBy(MyGeometry, -MouseEvent.GetWheelDelta() * GetNotchDistance(), EAllowOverscroll::No, bAnimateWheelScrolling);
...
bool SRTSDKScrollBox::ScrollBy(const FGeometry& AllottedGeometry, float LocalScrollAmount, EAllowOverscroll Overscrolling, bool InAnimateScroll)
{
	Invalidate(EInvalidateWidget::LayoutAndVolatility);
...
FCursorReply SRTSDKScrollBox::OnCursorQuery(const FGeometry& MyGeometry, const FPointerEvent& CursorEvent) const
{
	if (IsRightClickScrolling())
	{
...
FReply SRTSDKScrollBox::OnTouchEnded(const FGeometry& MyGeometry, const FPointerEvent& InTouchEvent)
{
	CachedGeometry = MyGeometry;
...
void SRTSDKScrollBox::OnMouseCaptureLost(const FCaptureLostEvent& CaptureLostEvent)
{
...
FNavigationReply SRTSDKScrollBox::OnNavigation(const FGeometry& MyGeometry, const FNavigationEvent& InNavigationEvent)
{
	...
	TPanelChildren<SRTSDKScrollBox::FSlot>& Children = ScrollPanel->Children;
	for (int32 SlotIndex = 0; SlotIndex < Children.Num(); ++SlotIndex)
	{
...
void SRTSDKScrollBox::OnFocusChanging(const FWeakWidgetPath& PreviousFocusPath, const FWidgetPath& NewWidgetPath, const FFocusEvent& InFocusEvent)
{
	if (ScrollWhenFocusChanges != ERTSDKScrollWhenFocusChanges::NoScroll)
	{
		if (NewWidgetPath.IsValid() && NewWidgetPath.ContainsWidget(this))
		{
			ScrollDescendantIntoView(NewWidgetPath.GetLastWidget(), ScrollWhenFocusChanges == ERTSDKScrollWhenFocusChanges::AnimatedScroll ? true : false, NavigationDestination, NavigationScrollPadding);
		}
	}
}

TSharedPtr<SWidget> SRTSDKScrollBox::GetKeyboardFocusableWidget(TSharedPtr<SWidget> InWidget)
{
...
int32 SRTSDKScrollBox::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
void SRTSDKScrollBox::ScrollBar_OnUserScrolled(float InScrollOffsetFraction)
{
	const float ContentSize = GetScrollComponentFromVector(ScrollPanel->GetDesiredSize());
	const FGeometry ScrollPanelGeometry = FindChildGeometry(CachedGeometry, ScrollPanel.ToSharedRef());
...
FSlateColor SRTSDKScrollBox::GetStartShadowOpacity() const
{
...
FSlateColor SRTSDKScrollBox::GetEndShadowOpacity() const
{
...
bool SRTSDKScrollBox::CanUseInertialScroll(float ScrollAmount) const
{
...
EAllowOverscroll SRTSDKScrollBox::GetAllowOverscroll() const
{
	return AllowOverscroll;
}

void SRTSDKScrollBox::SetAllowOverscroll(EAllowOverscroll NewAllowOverscroll)
{
...
void SRTSDKScrollBox::SetAnimateWheelScrolling(bool bInAnimateWheelScrolling)
{
	bAnimateWheelScrolling = bInAnimateWheelScrolling;
}

void SRTSDKScrollBox::SetWheelScrollMultiplier(float NewWheelScrollMultiplier)
{
	WheelScrollMultiplier = NewWheelScrollMultiplier;
}

void SRTSDKScrollBox::SetScrollWhenFocusChanges(ERTSDKScrollWhenFocusChanges NewScrollWhenFocusChanges)
{
	ScrollWhenFocusChanges = NewScrollWhenFocusChanges;
}

void SRTSDKScrollBox::BeginInertialScrolling()
{
	if (!UpdateInertialScrollHandle.IsValid())
	{
		bIsScrolling = true;
		bIsScrollingActiveTimerRegistered = true;
		UpdateInertialScrollHandle = RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SRTSDKScrollBox::UpdateInertialScroll));
		Invalidate(EInvalidateWidget::LayoutAndVolatility);
	}
}

void SRTSDKScrollBox::EndInertialScrolling()
{
	bIsScrolling = false;
	bIsScrollingActiveTimerRegistered = false;
	Invalidate(EInvalidateWidget::LayoutAndVolatility);
