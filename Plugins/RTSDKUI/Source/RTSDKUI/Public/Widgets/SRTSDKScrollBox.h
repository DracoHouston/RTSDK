...
class SRTSDKScrollPanel;

/** Where to scroll the descendant to */
UENUM(BlueprintType)
enum class ERTSDKDescendantScrollDestination : uint8
{
	/**
...
UENUM(BlueprintType)
enum class ERTSDKScrollWhenFocusChanges : uint8
{
...
/** SRTSDKScrollBox can scroll through an arbitrary number of widgets. */
class RTSDKUI_API SRTSDKScrollBox : public SCompoundWidget
{
public:
	/** A Slot that provides layout options for the contents of a scrollable box. */
	//class RTSDKUI_API FSlot : public TSlotBase<FSlot>, public TSupportsContentPaddingMixin<FSlot>
	//{
	//public:
	//	FSlot()
	//		: TSlotBase<FSlot>()
	...
	SLATE_BEGIN_ARGS(SRTSDKScrollBox)
		...
		, _NavigationDestination(ERTSDKDescendantScrollDestination::IntoView)
		, _NavigationScrollPadding(0.0f)
		, _ScrollWhenFocusChanges(ERTSDKScrollWhenFocusChanges::NoScroll)
		, _OnUserScrolled()
	...
	/** Custom scroll bar */
	SLATE_ARGUMENT( TSharedPtr<SRTSDKScrollBar>, ExternalScrollbar )
	...
	SLATE_ARGUMENT(ERTSDKDescendantScrollDestination, NavigationDestination);
	...
	SLATE_ARGUMENT(ERTSDKScrollWhenFocusChanges, ScrollWhenFocusChanges);
	...
	SLATE_ARGUMENT(bool, MovesNotched);

	SLATE_ARGUMENT(float, NotchDistance);
	 ...
	 */
	void ScrollDescendantIntoView(const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll = true, ERTSDKDescendantScrollDestination InDestination = ERTSDKDescendantScrollDestination::IntoView, float Padding = 0);
	...
	void SetNavigationDestination(const ERTSDKDescendantScrollDestination NewNavigationDestination);

	void SetScrollWhenFocusChanges(ERTSDKScrollWhenFocusChanges NewScrollWhenFocusChanges);
	...
	void SetMovesNotched(bool InMovesNotched);

	void SetNotchDistance(float InNotchDistance);
public:
...
protected:
	float LastViewFraction;

	void OnClippingChanged();

	/** Builds a default Scrollbar */
	TSharedPtr<SRTSDKScrollBar> ConstructScrollBar();
	...
	bool InternalScrollDescendantIntoView(const FGeometry& MyGeometry, const TSharedPtr<SWidget>& WidgetToFind, bool InAnimateScroll = true, ERTSDKDescendantScrollDestination InDestination = ERTSDKDescendantScrollDestination::IntoView, float Padding = 0);

	/**
	 * Scroll the view by ScrollAmount given its currently AllottedGeometry.
	 *
	 * @param AllottedGeometry  The geometry allotted for this SRTSDKScrollBox by the parent
	 * @param ScrollAmount
	 * @param InAnimateScroll	Whether or not to animate the scroll
...
protected:
	...
	TSharedPtr<class SRTSDKScrollPanel> ScrollPanel;

	/** The scrollbar which controls scrolling for the scrollbox. */
	TSharedPtr<SRTSDKScrollBar> ScrollBar;
	...
	ERTSDKDescendantScrollDestination NavigationDestination;

	/** Scroll behavior when user focus is given to a child widget */
	ERTSDKScrollWhenFocusChanges ScrollWhenFocusChanges;

	/**	The current position of the software cursor */
	FVector2D SoftwareCursorPosition;

	/** Fired when the user scrolls the scrollbox */
	FRTSDKOnUserScrolled OnUserScrolled;
	...
	float NotchDistance = 32;
};

class SRTSDKScrollPanel : public SPanel
{
public:

	SLATE_BEGIN_ARGS(SRTSDKScrollPanel)
	{
	...
	void Construct(const FArguments& InArgs, const TArray<SRTSDKScrollBox::FSlot*>& InSlots);

	void Construct(const FArguments& InArgs, TArray<SRTSDKScrollBox::FSlot::FSlotArguments> InSlots);
...
public:
	...
	TPanelChildren<SRTSDKScrollBox::FSlot> Children;
...
private:
	...
	float ArrangeChildVerticalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SRTSDKScrollBox::FSlot& ThisSlot, float CurChildOffset) const;
	float ArrangeChildHorizontalAndReturnOffset(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren, const SRTSDKScrollBox::FSlot& ThisSlot, float CurChildOffset) const;
