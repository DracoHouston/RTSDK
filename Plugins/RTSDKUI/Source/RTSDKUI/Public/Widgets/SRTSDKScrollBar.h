...
DECLARE_DELEGATE_OneParam(
	FRTSDKOnUserScrolled,
	float );	/** ScrollOffset as a fraction between 0 and 1 */


class SScrollBarTrack;

class RTSDKUI_API SRTSDKScrollBar : public SBorder
{
public:

	SLATE_BEGIN_ARGS( SRTSDKScrollBar )
		...
		SLATE_EVENT( FRTSDKOnUserScrolled, OnUserScrolled )
		...
		SLATE_ARGUMENT(float, NotchOffset)
	 ...
	 */
	void SetOnUserScrolled( const FRTSDKOnUserScrolled& InHandler );
	...
	bool GetMovesNotched();
	void SetNotchOffset(float InNotchOffset);
	float GetNotchOffset();
	float GetClosestNotchOffset(float InRawOffset);
